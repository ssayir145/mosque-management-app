...
exports.handler = withHandler(async (event) => {
  ...
  const rows = await sql`
    ...
    FROM users WHERE lower(email) = lower(${email}) AND role = 'caretaker'
  `;
  const user = rows[0];
  if (!user || !user.active) {
  ...
  const token = signToken({ sub: user.id, role: ROLES.CARETAKER, email: user.email, name: user.full_name });
